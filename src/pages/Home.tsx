        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-10">
            {industries.map((ind) => (
              ...
              >
                {/* Increased image size from w-20 h-20 to w-32 h-32 */}
                <div className="w-28 h-28 md:w-36 md:h-36 rounded-full overflow-hidden border-4 border-accent/30 shadow-xl">
                  <img src={ind.image} alt={ind.title} className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
                </div>
                <span className="text-primary-foreground font-display font-semibold text-base text-center max-w-[150px]">{ind.title}</span>
              </motion.div>
            ))}
          </div>
        </div>
